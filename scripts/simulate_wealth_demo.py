...
DEMO_TURNS = [
    {
        ...
        "verify": lambda r, cmds: (
            "total_aum_inr" in r
            and any(c["type"] == "show_portfolio_diagnostics" for c in cmds)
        ),
    },
    {
        "turn": 2,
        ...
        "action": lambda s: s.t_filter_products(
            category="Equity", tags=["Flexi Cap", "US Tech"], min_cagr_3y=18.0
        ),
        "verify": lambda r, cmds: (
            r.get("total_matches", 0) > 0
            and any(c["type"] == "filter_catalog" for c in cmds)
        ),
    },
    {
        "turn": 3,
        ...
        "action": lambda s: s.t_filter_products(
            category="Hybrid", tags=["All-Weather", "Auto-Rebalance"]
        ),
    ...
    },
    {
        ...
        "action": lambda s: s.t_simulate_portfolio(
            equity_pct=65,
            debt_pct=20,
            gold_pct=10,
            liquid_pct=5,
        ...
        "verify": lambda r, cmds: (
            r.get("projected_final_corpus_inr", 0) > 50000000
            and any(c["type"] == "update_simulation" for c in cmds)
        ),
    },
    {
        "turn": 5,
        ...
        "action": lambda s: (
            s.t_add_to_basket(
                "CPW-EQ-003",
                monthly_sip_amount_inr=35000,
                linked_goal="Retirement 2042",
            ),
            s.t_add_to_basket(
                "CPW-HB-002",
                monthly_sip_amount_inr=25000,
                linked_goal="Retirement 2042",
            ),
            s.t_add_to_basket(
                "CPW-DB-009", monthly_sip_amount_inr=20000, linked_goal="Education 2032"
            ),
            s.t_add_to_basket(
                "CPW-EQ-013", monthly_sip_amount_inr=20000, linked_goal="Global Hedge"
            ),
            s.t_view_basket(),
        )[-1],
        "verify": lambda r, cmds: (
            len(r.get("basket", [])) == 4 and r.get("total_monthly_sip_inr") == 100000
        ),
    },
    {
        "turn": 6,
        ...
        "verify": lambda r, cmds: (
            r.get("pdf_generated") is True
            and any(c["type"] == "proposal_ready" for c in cmds)
        ),
    },
    {
        "turn": 7,
        ...
        "action": lambda s: (
            s.t_request_mandate_authorization(),
            s.t_execute_mandate(otp="7701"),
        )[-1],
        "verify": lambda r, cmds: (
            r.get("status") == "success"
            and any(c["type"] == "mandate_executed" for c in cmds)
        ),
    },
]
