...
LANG_NAMES = {
    ...
    "kn": "Kannada", "bn": "Bengali", "mr": "Marathi", "gu": "Gujarati", "ml": "Malayalam", "pa": "Punjabi",
}
...
TURNS = [
    {"act": "filter", "note": "Discover monsoon rainwear (men's)",
    ...
    {"act": "add", "note": "Add the navy rain jacket",
    ...
    {"act": "complete", "note": "ONE turn → adds matching rain pants + waterproof shoes",
    ...
    {"act": "vto", "note": "Virtual try-on of the FULL look on Arjun (head-to-toe)",
    ...
    {"act": "checkout", "note": "Checkout — Arjun's saved Mumbai address + Mastercard shown",
    ...
    {"act": "promo", "note": "Apply promo DIRECT15 (total drops)",
    ...
    {"act": "payment", "note": "Place order → opens the SECURE CVV popup; shopper types the CVV there (never spoken)",
    ...
    {"act": "filter", "note": "OPTIONAL bonus — switch to women's catalog for a gift (identity stays male)",
...
async def main():
    ...
    ap.add_argument("--langs", default=",".join(PRIORITY),
                    help="comma list of language codes, or 'all' (default: en,hi,hinglish)")
    ap.add_argument("--repeat", type=int, default=1, help="repeat the whole run N times (variance)")
    ap.add_argument("--emit-md", action="store_true", help="write DEMO_SCRIPT_MULTILANG.md and exit")
    args = ap.parse_args()
    ...
    langs = list(LANG_NAMES) if args.langs == "all" else [l.strip() for l in args.langs.split(",") if l.strip()]
    bad = [l for l in langs if l not in LANG_NAMES]
    if bad:
        print(f"unknown languages: {bad} (known: {list(LANG_NAMES)})"); sys.exit(1)
    ...
    for lang in langs:
        ...
        print(f"  {LANG_NAMES.get(lang, lang):10s} {ok}/{n}  {'✅' if ok == n else '⚠️'}")
