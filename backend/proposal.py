...
def generate_proposal_pdf(proposal_data: dict, out_path: Path | str) -> str:
    ...
    elements.append(
        Paragraph("Strategic Portfolio Advisory & Rebalancing Mandate", title_style)
    )
    elements.append(
        HRFlowable(
            width="100%", thickness=2, color=primary_color, spaceBefore=2, spaceAfter=10
        )
    )
    ...
    aum_val = proposal_data.get("aum_inr", 7500000)
    risk_val = proposal_data.get("risk_profile", "Moderately Aggressive")
    if aum_val >= 10000000:
        aum_str = f"₹{aum_val / 10000000:.2f} Crores (₹{aum_val:,})"
    else:
        aum_str = f"₹{aum_val / 100000:.1f} Lakhs (₹{aum_val:,})"
    ...
    meta_data = [
        ...
        [
            Paragraph(f"<b>Managed AUM:</b> {aum_str}", body_style),
            Paragraph(f"<b>Risk Profile:</b> {risk_val}", body_style),
        ],
    ]
    ...
    elements.append(
        Paragraph("1. Executive Summary & Diagnostic Findings", section_style)
    )
    ...
    elements.append(
        Paragraph("2. Asset Allocation & Rebalancing Shifts", section_style)
    )
    alloc_headers = [
        "Asset Class",
        "Current Allocation",
    ...
    alloc_rows = [
        ...
        [
            ...
            "70.0% (₹52.5L)",
            "65.0% (₹48.75L)",
            "Trim large cap concentration; reallocate to Flexi & AI tech",
        ],
        [
            "Debt & Fixed Income",
            "15.0% (₹11.25L)",
            "20.0% (₹15.0L)",
            "Deploy in Target Maturity SDL 2030 & AAA Corp bonds",
        ],
        [
            "Commodities / Gold",
            "10.0% (₹7.5L)",
            "10.0% (₹7.5L)",
            "Maintain Sovereign Gold ETF allocation as inflation hedge",
        ],
        [
            "Liquid / Cash Buffer",
            "5.0% (₹3.75L)",
            "5.0% (₹3.75L)",
            "Maintain instant-access emergency reserve",
        ],
    ]
    ...
    elements.append(
        Paragraph("3. Recommended Advisory Basket & SIP Execution", section_style)
    )
    ...
    elements.append(
        KeepTogether(
            [
                Paragraph(
                    "Statutory Disclaimers & Regulatory Disclosures", section_style
                ),
                Paragraph(
