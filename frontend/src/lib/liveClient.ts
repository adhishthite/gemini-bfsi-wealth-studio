...
import { handleUiCommand } from "../ws";
...
export class LiveAvatar {
	...
	setTalking(on: boolean) {
		...
		s.set({
			live: { ...s.live, talking: on, status: on ? "listening" : "thinking" },
		});
	}
	...
	async start(avatar = "") {
		...
		this.ws = new WebSocket(
			`${proto}://${location.host}/ws/live?sid=${sid}${av}`,
		);
		this.ws.onmessage = (e) => this.onMessage(JSON.parse(e.data));
		this.ws.onclose = (e) =>
			this.cleanup(e.code === 1000 ? "ended" : `closed (${e.code})`);
			...
			this.ws?.send(
				JSON.stringify({
					realtimeInput: {
						mediaChunks: [
							{
								data: bytesToB64(new Uint8Array(i16.buffer)),
	...
	private playPcm(b64: string) {
		...
		if (this.pcmCount++ === 0)
			console.log(
				"[live] receiving PCM audio; outCtx state:",
				this.outCtx.state,
			);
	...
	private initMSE() {
		...
		ms.addEventListener("sourceopen", () => {
			...
			sb.mode = "sequence";
			this.sb = sb;
			if (this.initSeg && this.queue[0] !== this.initSeg)
				this.queue.unshift(this.initSeg);
	...
	private cleanup(status: string) {
		...
		if (!this.userStopped) {
			s.pushToast(
				this.lastError ||
					`Live session dropped (${status}). Tap Go Live to resume.`,
			);
		}
	}
}
