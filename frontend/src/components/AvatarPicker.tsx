    ...
    <div className="relative" ref={ref}>
      <button
        ...
        <span className="block h-16 w-16 rounded-full p-[2px] bg-brand-gradient">
          <img src={avatarImg(selectedAvatar)} alt={selectedAvatar}
            className="h-full w-full rounded-full object-cover object-top bg-white" />
        </span>
        <span className={`absolute bottom-0 right-0 h-3.5 w-3.5 rounded-full border-2 border-white ${connected ? "bg-emerald-500" : "bg-stone-300"}`} />
        {!disabled && (
      ...
      {open && (
        <div className="absolute left-0 top-[72px] z-50 w-64 glass rounded-xl2 shadow-lift p-2.5 animate-scale-in">
          <p className="px-1.5 pb-1.5 text-[11px] font-semibold uppercase tracking-wide text-stone-400">Choose your stylist</p>
          <div className="grid grid-cols-3 gap-2">
            {AVATARS.map((name) => (
              <button key={name} onClick={() => { selectAvatar(name); setOpen(false); }}
                ...
                <img src={avatarImg(name)} alt={name} className="h-16 w-16 rounded-lg object-cover" />
                <span className="text-[11px] font-medium text-brand-ink">{name}</span>
                {name === selectedAvatar && (
