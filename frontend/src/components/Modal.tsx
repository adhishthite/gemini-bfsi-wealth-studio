...
export default function Modal({
  open, onClose, children, title, maxW = "max-w-lg", dismissable = true,
}: {
  open: boolean; onClose: () => void; children: ReactNode; title?: string;
  maxW?: string; dismissable?: boolean;
}) {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      ...
      />
      <div className={`relative w-full ${maxW} glass rounded-xl2 shadow-lift animate-scale-in overflow-hidden`}>
        {(title || dismissable) && (
